...
%close all
clearvars -except id_max number_landmarks landmark_positions estimated_landmarks estimated_trajectory

assert(all(size(estimated_landmarks) == [2, number_landmarks]), "Estimated landmarks are not 2xN.");
...
trajectory_armse = compute_armse(simRobotTrail(1:2,:), estimated_trajectory(1:2,:));
